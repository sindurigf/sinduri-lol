...
- [ ] `npm run test:a11y` passes locally, not only in CI.
...
- [ ] Any new colour is a token from `src/styles/global.css`, measured against
      all three surfaces (`#131313`, `#1A1A1A`, `#0E0E0E`), with the ratios
      recorded in [ACCESSIBILITY.md](../ACCESSIBILITY.md) §5.
- [ ] No heading floor was raised without re-running `tests/reflow.spec.ts`.
- [ ] [ACCESSIBILITY.md](../ACCESSIBILITY.md) is updated in this PR if this
      change closes a gap, opens one, or invalidates a statement in it.
...
- [ ] Commits follow `type: Full sentence with a full stop.` and are signed off
      (`git commit -s`).
- [ ] No AI attribution in any commit message or in this description.
- [ ] No arbitrary values in components; every size, colour, shadow, and radius
      comes from a token.
- [ ] Documentation that this change makes wrong is corrected in the same PR.
