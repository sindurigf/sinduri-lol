...
**Scope.** Every route in `tests/routes.ts`, which is 25 of them, plus the
...
language checks are not. Five of the 25 pages carry real English: `/career`,
`/blog/open-source-is-not-just-code`, `/accessibility`, `/privacy` and `/404`.
The other 20 carry lorem, measured 2026-09-11 in ACCESSIBILITY.md gap 7.
...
otherwise.** Every number below is read off one machine in one session, with
nothing to diff it against. If the same page measures differently twice, the
cause is almost never the page:

- The browser. Headless Chromium overlays its scrollbar and gives a 320px
  viewport a 320px layout box, where headed Chrome draws a classic 15px bar and
  gives 305px: same CSS, same build, 15px apart. Record which browser and which
...
| ------------- | --------------------------------------------- | ------------ |
| Google Chrome | installed (153)                               | all but §6   |
...
- [ ] From there, one more Tab goes to the first control inside the page
      itself, not back into the header nav. On `/` that is the hero's pause
      control. That is the skip working. → SC 2.4.1
- [ ] Reload. Tab through the whole page. Order is: skip link → logo → About →
      Career → Blog → Get in touch → anything focusable in the page
      itself → the footer's pages (Home, About, Career, Blog) → Accessibility →
...
> `tests/reflow.spec.ts` asserts `scrollWidth <= clientWidth` on every route at
> **320px and 305px**, with and without the §7 spacing override, and at 640px
> and 1280px as well. **Still do this check by hand.** Resizing a viewport is
> not zooming: real 400% zoom brings a classic scrollbar, subpixel rounding,
> and the browser's own minimum font size, none of which the automated run
> sees.
>
...
> real 305px viewport and 288px at 320px, on every route. A card on a category
> route has 24px of padding at this width, not 40px, leaving a 209px content
> box.
>
...
> so the symptom of a bad floor is a heading cut mid-word, not a scrollbar. The
> suite measures every heading word against its own box as well, but the next
> check is still the one your eyes do.
...
- [ ] Tab through the page. **The sticky 96px header never covers the element
      ...
      → SC 2.4.11
- [ ] Repeat on `/blog/open-source-is-not-just-code`, the longest page.
      → SC 1.4.10, 2.4.11
...
**Which browser. Firefox, except for one step.** The rest of this document uses
Chrome. Orca is built and tested primarily against Firefox on Linux and its
Chromium support is weaker, so an unexpected result in Chrome may reflect the
Orca-Chromium pairing rather than this site's markup. Everything here runs in
**Firefox**: the skip link (6.5), the header, footer, landmarks and headings
(6.6), the MobileMenu dialog (6.7), the blog post's heading order (6.8) and the
404 page (6.9). A result from Firefox can be acted on as it stands.

**Chromium is used for exactly one step, 6.4, the uppercase question.**
Chromium is the only engine that puts the string produced by `text-transform`
into the accessibility tree; Firefox and WebKit are reported to leave the name
as authored, so asking Firefox how an all-caps accessible name is announced
answers nothing. Step 6.4 runs in both: Firefox as the control, Chrome for the
only reading that can settle it. Nothing else here needs Chrome, and anything
strange in the Chrome half of 6.4 is a question, not a finding.
...
D-Bus (`org.a11y.Status IsEnabled`), so
`gsettings set org.gnome.desktop.interface toolkit-accessibility true` is a
GTK-era fallback rather than a requirement. If nothing is announced anyway, in
this order: restart the browser while Orca is already running; set
`toolkit-accessibility true` and restart it again. Set it back to `false`
afterwards if the desktop feels slow.

Silence is more likely in Chrome than in Firefox. If Chrome stays silent
through both fallbacks, leave the 6.4 blanks empty and say so: an unanswered
question recorded as unanswered is worth more than one answered by the wrong
engine.
...
This is the open half of a known issue and the only reason to run the pass in a
particular order: three files are waiting on the answer.

`ARCHITECTURE.md` [Uppercase](../ARCHITECTURE.md#uppercase) and the design system skill's
...
exposes the accessible name `"ABOUT"`. Both record that Firefox and WebKit do
not transform it, and both flag what a reader actually _says_ for such a name
as unverified. That guess is what this step replaces.

**Why Firefox alone cannot answer it.** Firefox never produces the input
condition being asked about, an all-caps accessible name, so hearing "career"
there measures Firefox's name rather than Orca's handling of caps. Chromium is
the only engine on this machine that puts `"CAREER"` into the tree. Firefox is
...
**No outcome changes the practice.** `text-transform` in CSS stays and sentence
case in the markup stays, whatever Orca says; only the stated justification
changes, and only if the evidence supports it. The rule was never justified by
the announcement: the design system skill is explicit that the earlier "it
keeps the accessible name in sentence case" defence is false. It rests on three
things this test cannot touch. Chromium transforms the accessible name and
Firefox and WebKit do not, so sentence case is the only safe input. The content
stays editable, copy-pasteable and searchable as written. And machines reading
the DOM get the true string. Typing `ABOUT` into the markup makes all three
worse and fixes nothing.
...
- [ ] **The reading cursor moved, not just focus.** Press `Down` immediately.
      A pass reads something from inside `main`: on `/`, the hero's pause
      control or the `h1`, "Sinduri Guntupalli". A line from the header is a
      fail: the cursor never left the top.
      Heard: `___________________________________________` → SC 2.4.1
- [ ] Press `Tab` once. A pass lands on the hero's pause control, the first
      control inside `main` on `/` (measured 2026-09-11). A header nav link
      means the skip did not take. With reduced motion set the hero renders no
      pause control, so the stop is the next focusable thing down the page.
      Heard: `___________________________________________` → SC 2.4.1
...
its nav is not in the accessibility tree; and the no-JavaScript fallback nav in
`Header.astro` is `display: none` while scripting is on, so it is not there
...
menu is present at any width below that; 320px is what the rest of this
document is calibrated against. Narrowing the window itself is cleanest. If it
will not go that narrow, use Firefox's Responsive Design Mode (`Ctrl+Shift+M`)
at 320. **Detach the developer tools into their own window first** (the tools'
...
- [ ] `Alt+Shift+M` with the panel open lists **one** navigation, unnamed,
      ...
      `landmark-unique` regression this markup was changed to avoid. The
      automated suite runs that rule only with the panel closed, in the
      best-practice block of `tests/a11y.spec.ts`; the menu-open scan uses the
      WCAG tags alone, so no test has run it against the open panel.
      Listed: `__________________________________________` → SC 1.3.1
...
- [ ] The category link reads "open source" and appears in `Alt+Shift+K` with
      no surrounding context. Judge it there: does it say it is a category
      filter? If not, that is a link-text finding, not a pass.
      Noted: `___________________________________________` → SC 2.4.4
...
- [ ] `Ctrl+Home`, then `Down` through the page. Within the first few lines it
      ...
      listener's end.
      Heard: `___________________________________________` → SC 1.3.1
- [ ] `Orca+Slash` reads the window title. A pass contains "Page not found".
      Heard: `___________________________________________` → SC 2.4.2
- [ ] `Alt+Shift+H` lists exactly one heading: the `h1` "Page not found".
      Listed: `__________________________________________` → SC 1.3.1, 2.4.6
...
read through one engine, on one machine, by one person. NVDA and VoiceOver pair
announcements with browsers differently, and neither is available here; JAWS is
not either. A page Orca reads correctly has been shown to work for Orca users
in Firefox, not for everyone using a screen reader.
...
| The 6.4 answers            | `ARCHITECTURE.md` → Uppercase; `.claude/skills/sinduri-design-system/SKILL.md` → `#### Why, accurately`; `ACCESSIBILITY.md` §6 → "How uppercase is announced"; `ACCESSIBILITY.md` §7 gap 4. |
...
Outcomes 1 and 2 both replace a flagged assumption with a measurement in three
files, and nothing else changes. If instead a reading turns out to be genuinely
bad for a listener, that **additionally** opens a gap against the visual
decision to uppercase at all. That is a design question for Sinduri: it would
mean dropping `text-transform` from `.label` and the headings, which changes
how the site looks. Do not make that change as a side effect of running this
test. Either way, the "unverified" sentence in ARCHITECTURE.md and in the skill gets
replaced with what was actually heard, named by browser and version, with the
Capitalization style value alongside it.
...
**Firefox and WebKit are automated.** `playwright.config.ts` defines a
`firefox` project alongside `chromium`, and a `webkit` project in CI, so the
whole suite runs in three engines. Firefox 153.0 passed every one of the then
487 tests on the first attempt, on 2026-09-05, with no source change and no
browser-conditional assertion anywhere in `tests/`. `<dialog>` and
`showModal()` behave, which is what this section was written to worry about.
...
- [ ] WebKit: **automated in CI.** The first run, in Playwright's container
      before the project was defined, passed 487 tests in 44.1s; the suite is
      963 tests per browser project as of 2026-09-11, and the config records
      one WebKit failure in CI, run 34528550453 on 2026-09-10. A real Safari
      pass on real hardware is still worth doing if anyone has a Mac. Headless
      WebKit is not Safari, and the gap is widest in exactly the places this
      checklist cares about: VoiceOver, and how the platform draws focus.
...
`--user` is load-bearing: without it the image runs as root over the
bind-mounted tree, `npm ci` leaves it root-owned, and the next local command
fails on a container that exited half an hour earlier. The measurement, and the
recovery if you hit it anyway, are in `playwright.config.ts` beside the other
copy of this command. Correct both copies or neither.
...
- [ ] **The page gutter is present at every width.** Narrow to 320px and
      ...
      edges, on a page such as `/about` and on a blog route. A route whose text
...
## 10. Exploratory tools, and what a trial of one produced
...
1. `sr-duplicate-landmark`, "banner (2x), main (3x)", graded moderate. **False.**
   ...
   page-audit snapshots the run took, 3 on desktop and 4 on mobile, so it is
...
3. `focus-appearance-weak`, WCAG 2.4.13, informative. Graded AAA and so not a
   ...
   (across 1672 controls at both widths the lowest is 10.60:1, measured
   2026-09-08), but the dimension was genuinely untested and `global.css` had
   claimed 11.32 / 10.60 / 11.76 in a comment with nothing behind it.
   `tests/focus.spec.ts` now measures it, for SC 1.4.11.
...
## 11. Items mined from external checklists
...
description, since there is no video or audio; dragging movements, accessible
...
- **Language, not structure.** Reading order, in-page heading structure and
  ...
  there is a defect underneath that: lorem ipsum is Latin inside a `lang="en"`
  document, so a screen reader announces it with English pronunciation rules.
  That is ACCESSIBILITY.md gap 7 and it is fixed by finishing the writing.
