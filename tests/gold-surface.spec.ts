 ...
 * and in the design system skill.
 *
 ...
 *      either direction, and it is what keeps the tables in ARCHITECTURE.md,
 ...
 *   4. Every control in a gold section is delimited against the ground, by its
 *      opaque fill or, when the fill is transparent, by its border. Also a
 *      fixture; see `invisibleControls` in tests/contrast.ts for why it is two
 *      arms.
 ...
 * a page has no gold section. Every route in `GOLD_ROUTES` is now asserted to
 * have one with the expected number of controls in it.
 *
 * VERIFIED NOT TO BE VACUOUS. Each assertion was broken on purpose and the
 * failure checked:
 *
 *   change `.skip-link` to `text-muted`     test 2 fails on every route at
 *                                           1.03:1
 ...
 * rather than a fixture: a temporary route with a .surface-gold hero, added to
 * ROUTES and run both ways. With .btn-gold-primary it passed; with
 * .btn-primary swapped in it failed, reporting the fill as rgb(255, 192, 0) on
 * rgb(255, 192, 0). The same page in the same state returned "No accessibility
 * violations found" from AccessLint with AAA enabled, which is the whole
 * reason this check exists.
 *
 * Its BORDER arm was verified on the fixture in test 4, twice: the secondary's
 * border set to border-gold reports all four edges at 1.00:1, and setting only
 * its border-bottom-color reports that one edge. The second is why all four
 * edges are tested rather than one. With only the bottom edge painted gold,
 * every other assertion in this file passed, the button test included, because
 * its `borderOnGold` reads borderTopColor and the top edge was still #131313.
 *
 * The two-tone ring in test 5 was verified two ways, and the pair of results
 * is the point of that phase. Deleting `.btn-gold-primary:focus-visible` fails
 * test 5 on its first assertion, reporting the outline rgb(19, 19, 19) at
 * 1.00:1 with zero offset, which is the state this file was in before the
 * inner ring existed. Setting `outline-offset: 0` on `.surface-gold` fails the
 * button test's offset assertion and test 5 PASSES, because the inner ring is
 * still 18.58 against the fill. That second result is what "the offset
 * assertion is redundant rather than load-bearing" means, measured rather than
 * asserted in prose.
 *
 * Two ways test 6 can lie, both guarded in the code below. `getComputedStyle`
 * returns a LIVE declaration, so a value read after the element is blurred is
 * the resting value, and `document.activeElement === el` in the returned
 * object literal is evaluated after the blur too. Both are read into plain
 * numbers while the element still has focus.
 */
 ...
 * correct while none existed and is a trap now: if a hero lost the class, or
 * its buttons were swapped for something else, every route would keep
 * returning `[]` and the check would go quietly vacuous rather than fail.
 *
 * A map rather than a single route, because more than one page uses the class
 * and the non-vacuity guard below only fires on routes named here.
 ...
 *
 * The same token/hex/ratio table is written out in three Markdown files, which
 * is three chances to retone a colour in the CSS and update two of them.
 * Nothing about the third going stale is visible: it is a number in a sentence
 ...
 * table in ARCHITECTURE.md, which measures one colour against four grounds and puts two
...
test.describe('the gold surface exception', () => {
   ...
   * `gold-muted` to `#3A3021` in ARCHITECTURE.md alone fails this test naming that file
    ...
    for (const [index, row] of measured.entries()) {
      ...
      expect(
        ...
        `${row.variable} is ${row.value}, which measures ${row.ratio} against ` +
          ...
          `table in ARCHITECTURE.md, ACCESSIBILITY.md and the design system skill quotes ` +
   ...
   *
   * The offset assertion in the button test above stays, redundant rather than
   * load-bearing. This test is what makes it redundant.
   */
  test('.btn-gold-primary keeps a visible focus ring whatever the offset does', async ({
     ...
     * sees names the actual consequence, that the indicator is invisible
     * against the control it marks, rather than an intermediate fact about how
     * many box-shadow layers there are.
     */
    expect(
  ...
  /**
   * The gold surface on real markup, at the two reflow widths: the buttons a
