 ...
 * floor for large text, which the standard defines as 18pt, or 14pt bold: 24px
 * and 18.66px at this site's 16px root. NON_TEXT is SC 1.4.11, which covers
 * the boundary of a control and the focus indicator that marks its state.
 * AAA_TEXT is SC 1.4.6 and is informative here: this site targets AA, and the
 * gold tokens simply clear the higher bar.
 */
 ...
 *
 * A string and not a module because every consumer runs it inside
 ...
 *
 * Shared because tests/gold-surface.spec.ts, where it was written and where
 * most of it is still used, and tests/contrast-incomplete.spec.ts need the
 * same `parse`, `luminance`, `ratio`, `over` and `effectiveBackground`. A
 * second copy of a WCAG contrast implementation is the kind of duplication
 * that drifts silently: two files would disagree by a rounding rule and both
 * would keep passing. One definition, two importers.
 *
...
export const PAGE_HELPERS = `
   ...
   *
   * BOTH OF THOSE NUMBERS PREDATE THE WALK FIX AND NEITHER HAS BEEN RE-TAKEN.
   * 572 was the walk's control count before the blog added twelve routes and
   * before it stopped truncating on repeated link text; the same walk now
   * reaches 1672, and tests/focus.spec.ts records how that gap splits. 138 was
   * counted against the 572, so re-deriving it from 1672 by proportion would
   * be a number nobody measured. The ratio is what the argument rests on and
   * it is not close, so the pair is left as the dated observation it was
