...
would have to set its own headers in code.
...
Adding a `<form>` with a real submission target requires widening this directive
in the same commit, or the browser blocks the submission, silently: nothing
appears on the page and only the console carries the refusal. It lands hardest
on the people a contact form is for.

Widen it to the exact origin the form posts to, never `*`, and add an assertion
to `tests/headers.spec.ts` in the same commit. `object-src`, `base-uri` and
`frame-ancestors` stay `'none'`, which records that the feature is unused rather
than that it is banned.
...
- **The contact form.** See the CSP section above and
  [ACCESSIBILITY.md](ACCESSIBILITY.md) §7 gap 1.
