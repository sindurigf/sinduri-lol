/*
 * Fails if a Markdown file links to a relative path that does not exist.
 *
 * The documentation here is cross-referenced, and a broken relative link fails
 * silently on GitHub: it renders as an ordinary link and 404s on click. The
 * pull request template is the sharpest case, since it sits in .github/ and
 * reaches its targets one directory up.
 *
 * External URLs are not fetched: a checker that reaches the network fails for
 ...
 * in the current file.
 *
 * A leading `/` is a site route, which is the case worth spelling out: it
